...
ap.add_argument('-b', '--backgroundImages', required=True, help="Path to background images")
...
back = np.load(args['backgroundImages'])
hardExamples = []
orientation = 9
pixelsPerCell = (6, 6)
cellsPerBlock = (3, 3)
...
def processInput(i):
    windows = []
    for resized in pyramid(back, scale=1.2):
...
np.array(refined).dump(args['saveAs'])
